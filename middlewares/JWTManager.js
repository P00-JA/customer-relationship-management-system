...
const JWTManager = {
  createToken(payload, refresh = false) {
    const accessToken = jwt.sign(payload, process.env.SECRETMNGR, {
      expiresIn: 60 * 60,
    });
    return {
      access_token: accessToken,
      refresh_token: refresh? jwt.sign(payload, process.env.SECRETMNGR, {
  ...
  verifyToken(token) {
    try {
      const decoded = jwt.verify(token, process.env.SECRETMNGR);
      return decoded;
    } catch (error) {
      return false;
    }
