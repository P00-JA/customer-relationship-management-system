...
const JWTCustomer = {
  createToken(payload, refresh = false) {
    const accessToken = jwt.sign(payload, process.env.SECRETCUS, {
      expiresIn: 60 * 60,
    });
    return {
      access_token: accessToken,
      refresh_token: refresh? jwt.sign(payload, process.env.SECRETCUS, {
  ...
  verifyToken(token) {
    try {
      const decoded = jwt.verify(token, process.env.SECRETCUS);
      return decoded;
    } catch (error) {
      return false;
    }
